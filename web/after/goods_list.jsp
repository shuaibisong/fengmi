...
<script>
    ...
    function showAdd(){
        location.href="/hangsong_fengmi_war_exploded/after/add_goods.jsp?num=0";
    }
    function checkAll(){
        var goodsids=document.getElementsByName("goodsid");
        if(allnum == 0){
    ...
    function delectAll(){
        if(confirm("确定删除吗？")){
            ...
            window.location.href="/hangsong_fengmi_war_exploded/DeletGoodsServlet?goodsid="+ids;
        }
    ...
    function showName(obj, id) {
        ...
        $("#typeName").val(name);
        $("#typeId").val(id);
        <%--document.getElementById("show").value.innerHTML=${goods.typename};--%>
    }
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse"
            ...
            <ul class="nav navbar-nav">
                <li class="active"><a href="<%=path%>/FuzzySelectGoods">商品管理 <span
        ...
        <div class="col-md-6" style="text-align: right; padding-right: 25px;">
            <form class="form-inline" action="<%=path%>/FuzzySelectGoods">
                ...
                <input type="hidden" class="form-control" id="typeId" name="typeId"
                       value="${goods.typeId}">
                <input type="hidden" class="form-control" disabled id="typeName"
        ...
        <table style="text-align: center; margin-right: 10px; width: 99%"
            ...
            <c:forEach items="${list}" var="goods" varStatus="obj">
                <tr>
                    ...
                    <td>${goods.price}</td>
                        <%-- ${goods.imgPath} --%>
                    <td><img src="images/my/${goods.imgPath}"
            ...
            <td><a name="goodsid" href="<%=path%>/DeletGoodsServlet?goodsid=${goods.id}" onclick="if(confirm('是否确定删除'+'${goods.goodsName}'+'？')==false)return false">删除</a></td> --%>
                    ...
                    <td><a href="<%=path%>/after/add_goods.jsp?id=${goods.id}&num=1"
                    ...
                    <td><a class="glyphicon glyphicon-trash" color="#5BC0DE"
                           ...
                           href="<%=path%>/DeletGoodsServlet?goodsid=${goods.id}"
...
<div class="container">
    <nav aria-label="..." class="navbar-right" style="margin-right: 15px">
        <ul class="pagination">
            <li><a class=" btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=1&goodsname=${goodsname}" aria-label="Previous">
            ...
            %>
            <li><a class=" btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo-1}&typeId=${typeId}&goodsname=${goodsname}"
            ...
            <li><a class="btn"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo+1}&typeId=${typeId}&goodsname=${goodsname}">${pageNo}</a></li>
            <li><a class="btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo+1}&typeId=${typeId}&goodsname=${goodsname}"
            ...
            <li><a class=" btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo}&typeId=${typeId}">${pageNo}</a></li>
            <li class="disabled"><a href="#" aria-label="Previous">»<span
                    class="sr-only">(current) </span></a></li>
            <%
                }
